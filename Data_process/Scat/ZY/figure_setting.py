...
def my_zstack_scats(ax, title=''):
    ...
    set_tick(ax, ticks_xlabel=np.arange(500, 901, 100), ticks_ylabel=np.arange(0, 19, 1), show_xlabel_every_ticks=2, show_ylabel_every_ticks=2,
...
def my_maxint_ints(ax, title=''):
    ...
    set_tick(ax, ticks_xlabel=np.arange(0, 19, 1), show_xlabel_every_ticks=2,
             yaxis_use_log_scale=False)  # Normalized

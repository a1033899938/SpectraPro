...
for iType, (process_type, root) in enumerate(zip(process_types, roots)):
    ...
    for iTile, full_tile_key in enumerate(full_tile_keys):
        ...
        for iParticle, full_particle_key in enumerate(full_particle_keys):
            ...
            ax2_21.imshow(img) if if_plot else None
            ax2_21_inset.imshow(img_gray)  if if_plot else None
            ...
            ax2_11.pcolor(wav, positions, scats, cmap="coolwarm") if if_plot else None
            "wav-maxz曲线"
            ...
            for iSection, section in enumerate(fit_section):
                ...
                ax.plot(wav, maxz, "-", color="lightblue", label="Data - Focused Position") if if_plot else None
                ax.plot(x_for_fit, fitted_curve, "--", color=section_colors[iSection], linewidth=2,
                         label=f"Fitted Line: {label}") if if_plot else None
                content = f"Slope(um/nm): \n{popt[0]:.3f}"
                ax.text(x=section_center, y=8.5, s=content, fontsize=15, ha='center', va='top',
                         color=section_colors[iSection], weight='bold') if if_plot else None
                # 取出第一段曲线的拟合直线
                if iSection == 0:
            ...
            #                          fr"Images2//{process_type}_Tile{iTile}_p{iParticle}.png")
...
save_path = os.path.join(os.path.dirname(filepath),
                              fr"Images3//Statics_{cal_error_func}.png")

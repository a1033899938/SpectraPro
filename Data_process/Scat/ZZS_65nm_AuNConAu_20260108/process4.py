...
the_save_path = r"D:\ExpData\NP_scattering_Dispersion\ZZS_65nm_AuNConAu_20260108"
filepaths = [r"D:\ExpData\NP_scattering_Dispersion\ZZS_65nm_AuNConAu_20260108\ZZS_70nm_AuNConAu_circle_20260108.h5",
             r"D:\ExpData\NP_scattering_Dispersion\ZZS_65nm_AuNConAu_20260108\ZZS_70nm_AuNConAu_circle_20260108.h5",
             r"D:\ExpData\NP_scattering_Dispersion\ZZS_65nm_AuNConAu_20260108\ZZS_70nm_AuNConAu_20260108.h5"]
process_types = ["before_achro", "after_achro_lens1", "after_achro_lens2"]
roots = ["OceanOpticsSpectrometer/20260108/before_achro",
         "OceanOpticsSpectrometer/20260108/after_achro_old_2",
         "OceanOpticsSpectrometer/20260108_2/after_achro"]
...
wav_range = [450, 1000]
fit_section = [[500, 656.3], [656.3, 730], [730, 1000]]

section_colors = ["#0E8585", "#830783", "#FFA500"]
type_colors = ["blue", "red", "orange"]
...
if_plot1 = True
if_plot2 = True
if_import_data = False

"""处理图片"""
fig = plt.figure(figsize=(12 * 3, 8 * 3), dpi=200)
ax11 = fig.add_subplot(331)  # Fitted Slope
ax12 = fig.add_subplot(332)  # Recon. tau
ax13 = fig.add_subplot(333)  # Recon. Time

ax21 = fig.add_subplot(334, sharex=ax11)
ax22 = fig.add_subplot(335, sharex=ax12)
ax23 = fig.add_subplot(336, sharex=ax13)

ax31 = fig.add_subplot(337, sharex=ax11)
ax32 = fig.add_subplot(338, sharex=ax12)
ax33 = fig.add_subplot(339, sharex=ax13)
...
bgd_color3 = (1.0, 0.6, 0.0, 0.1)
bgd_colors = [bgd_color1, bgd_color2, bgd_color3]

axess = [axes1, axes2, axes3]
particle_number = [0, 0, 0]
...
# ax000 = fig.add_subplot(111)
# with h5py.File(filepaths[0], "r") as f:
#     sp = f["OceanOpticsSpectrometer/ref_100ms_subtrate_bgd_100ms_0"]
...
# ax000.plot(wav, ref_minus_bgd, label="2R:8T BS")
#
# with h5py.File(filepaths[2], "r") as f:
#     sp = f["OceanOpticsSpectrometer/ref_100ms_subtrate_bgd_100ms_1"]
#     bgd = np.array(sp.attrs["background"])
#     bgd_time = sp.attrs["background_int"] / 1000
#     bgd = bgd / bgd_time
#
...
#     ref = ref / ref_time
# ref_minus_bgd = ref - bgd
# ax000.plot(wav, ref_minus_bgd, label="5R:5T BS")
# my_graph(ax000, title="Reference", xtick=np.arange(400,1101, 200), xlabel="Wavelength(nm)", ylabel="Intensity(cts)")
# plt.show()
# input()

for iType, (process_type, root, filepath) in enumerate(zip(process_types, roots, filepaths)):
    # if iType != 2:
    #     continue
    ...
    if not if_import_data:
        ...
        for iTile, full_tile_key in enumerate(full_tile_keys):
            ...
            for iParticle, full_particle_key in enumerate(full_particle_keys):
                ...
                # if iType == 2 and iTile < 55:
                #     continue
                #
                # if particle_number[iType] >= 10:
                #     continue
                ...
                "拟合wav-maxz曲线"
                popts = []
                ax = ax2_11  if if_plot2 else None
                ...
                wav_idx1 = find_val_idx(wav, fit_section[2][0])
                wav_idx2 = find_val_idx(wav, fit_section[2][1])
                ...
                ax = ax2_12 if if_plot2 else None
                if if_plot2:
                ...
                if if_plot2:
                    ...
                    save_path = os.path.join(the_save_path,
    ...
    """导入结果"""
    if if_import_data:
        loadpath = os.path.join(the_save_path, "Images", f"{process_type}_Statics_{cal_error_func}.npz")
    ...
    for iSection in range(3):
        ...
        fit_stats(ax, counts, bins, x_hist=x_hist, sigma=0.02,
                  text_x=0.05, text_y=0.95-iSection*0.05, color=section_colors[iSection], decimal=4)
    ...
    if not if_import_data:
        savepath = os.path.join(the_save_path, "Images", f"{process_type}_Statics_{cal_error_func}")
...
my_wav_maxz_fit_popt_stats(ax31, title="", title_pad=0)
...
my_reconstrution_error_fit_popt_stats(ax32, title="", title_pad=0)
...
my_wav_maxz_required_time_stats(ax33, title="", title_pad=0)
save_path = os.path.join(the_save_path, "Images", f"Statics_{cal_error_func}.png")

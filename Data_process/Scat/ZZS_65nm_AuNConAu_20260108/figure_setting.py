...
def my_reconstruct_scat(ax, title_pad=17, title='', bbox_to_anchor=(1, 0.6)):
    ...
    set_tick(ax, ticks_xlabel=np.arange(500, 1001, 100), show_xlabel_every_ticks=2, yaxis_use_log_scale=False) # Normalized
    set_spines(ax)
...
def my_zstack_scats(ax, title_pad=17, title='', ytick=np.arange(-10, 11, 5)):
    ...
    set_tick(ax, ticks_xlabel=np.arange(500, 1001, 100), ticks_ylabel=ytick, show_xlabel_every_ticks=2, show_ylabel_every_ticks=2,
             yaxis_use_log_scale=False)  # Normalized
    set_spines(ax)

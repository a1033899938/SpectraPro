...
def my_graph1(ax, title=''):
    ...
    set_tick(ax, ticks_xlabel=np.arange(500, 1001, 200), show_xlabel_every_ticks=4, yaxis_use_log_scale=False) # Normalized
...
def my_graph3(ax, title=''):
    set_label_and_title(ax, title=f"{title}_histogram", xlabel='Slope(um/nm)', ylabel='Frequency')
    set_tick(ax)  # Normalized
    set_legend(ax, font_size=12, location='best')
    set_spines(ax)
    plt.tight_layout()

def my_graph4(ax, title=''):
    set_label_and_title(ax, title=f"{title}_histogram", xlabel='Require time(s)', ylabel='Frequency')
    set_tick(ax) # Normalized
    set_legend(ax, font_size=12, location='best')
    set_spines(ax)
    plt.tight_layout()
...
def my_reconstruct_scat(ax, title=''):
    ...
    set_tick(ax, ticks_xlabel=np.arange(400, 901, 100), show_xlabel_every_ticks=2, yaxis_use_log_scale=False) # Normalized
    set_spines(ax)
    plt.tight_layout()

def my_zstack_scats(ax, title=''):
    set_label_and_title(ax, title=title, xlabel='Wavelength(nm)', ylabel='z(step)')
    set_tick(ax, ticks_xlabel=np.arange(400, 901, 100), show_xlabel_every_ticks=2, show_ylabel_every_ticks=2,
             yaxis_use_log_scale=False)  # Normalized
    set_spines(ax)
    plt.tight_layout()

def my_maxint_ints(ax, title=''):
    set_label_and_title(ax, title=title, xlabel='z(step)', ylabel='Intensity(a.u.)')
    set_tick(ax, show_xlabel_every_ticks=2,
             yaxis_use_log_scale=False)  # Normalized
...
def my_maxz_range_stats(ax1, title=""):
    set_label_and_title(ax1, title=title, xlabel='z(step)', ylabel='Frequency')
    set_tick(ax1)

...
with h5py.File(filepath, "r") as f:
    root_folder = f["OceanOpticsSpectrometer/20251129"]

    for iType, process_type in enumerate(process_types):
        parent_folder = root_folder[process_type]
        ...
        bounds1 = [[0.03, -30],
                   [0.09, 10]]
        for iKey, key_1 in enumerate(sorted_keys_1):
            ...
            popt = None
            try:
                x_for_fit1, y_for_fit1 = choose_range(wav, np.array(max_positions), x1=400, x2=700)
                popt1, pcov1 = curve_fit(linear, x_for_fit1, y_for_fit1)
                fitted_curve1 = linear(x_for_fit1, *popt1)

                x_for_fit2, y_for_fit2 = choose_range(wav, np.array(max_positions), x1=700, x2=950)
                popt2, pcov2 = curve_fit(linear, x_for_fit2, y_for_fit2, p0=p01, bounds=bounds1)
                fitted_curve2 = linear(x_for_fit2, *popt2)
            except Exception as e:
                print(str(e))
            ...
            # single_axis_scan_map2d(ax)

            ax.plot(wav, max_positions, color=color)

            if popt1 is not None:
                ax.plot(x_for_fit1, fitted_curve1, '--', color="black")
                ax.plot(x_for_fit2, fitted_curve2, '--', color="black")
                ax.set_title(f"slope1: {popt1[0]:.2f}, slope2: {popt2[0]:.2f}")

            ax = axes[iKey][iType+2]
            scat_max = np.max(scats, axis=0)
            ax.plot(wav, scat_max)
plt.show()

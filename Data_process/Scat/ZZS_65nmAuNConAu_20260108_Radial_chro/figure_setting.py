...
def my_reconstruct_scat(ax, title_pad=17, title='', bbox_to_anchor=(1, 0.6)):
    ...
    set_tick(ax, ticks_xlabel=np.arange(500, 1101, 100), show_xlabel_every_ticks=2, yaxis_use_log_scale=False) # Normalized
    set_spines(ax)
...
def my_zstack_scats(ax, title_pad=17, title='', ytick=np.arange(-10, 11, 5)):
    ...
    set_tick(ax, ticks_xlabel=np.arange(500, 1101, 100), ticks_ylabel=ytick, show_xlabel_every_ticks=2, show_ylabel_every_ticks=2,
             yaxis_use_log_scale=False)  # Normalized
    set_spines(ax)
...
def my_thumb_image(ax, title=''):
    ...
    set_tick(ax, ticks_xlabel=np.arange(0, 1240, 200), ticks_ylabel=np.arange(0, 1080, 200),
             yaxis_use_log_scale=False)  # Normalized
    set_spines(ax)
    plt.tight_layout()
...
"wav_maxz"
def my_wav_maxz_fit_popt_stats(ax, title_pad=17, title=''):
    set_label_and_title(ax, title=title, xlabel=f'Fitted Slope(um/nm)', ylabel=f'Frequency', title_pad=title_pad)
    set_tick(ax, y_tick_integer=True)  # Normalized
    set_spines(ax)
    set_legend(ax, font_size=20, location='best')
    plt.tight_layout()

def my_reconstrution_error_fit_popt_stats(ax, title='', title_pad=17):
    set_label_and_title(ax, title=title, xlabel=f"Recon. Convergence Time Constant {My_Char.get('tau')} (s)", ylabel=f'Frequency', title_pad=title_pad)
    set_tick(ax, y_tick_integer=True)  # Normalized
    set_spines(ax)
    set_legend(ax, font_size=20, location='upper left')
    plt.tight_layout()

def my_wav_maxz_required_time_stats(ax, title='', title_pad=17):
    set_label_and_title(ax, title=title, xlabel=f'Recon. Required Time(s)', ylabel=f'Frequency', title_pad=title_pad)
    set_tick(ax, y_tick_integer=True)  # Normalized
    set_spines(ax)
    set_legend(ax, font_size=20, location="upper left")
    plt.tight_layout()
...
"""Radial"""
def my_radial_1(ax, title="", title_pad=17):
    set_label_and_title(ax, title=title, xlabel=f'Position of Collection Area(um)', ylabel=f'Fitted Slopes(um/nm)', title_pad=title_pad)
    set_tick(ax, y_tick_integer=False)  # Normalized
    set_spines(ax)
    set_legend(ax, font_size=17, location="lower left")
    plt.tight_layout()

def my_radial_2(ax, title="", title_pad=17):
    set_label_and_title(ax, title=title, xlabel=f'Position of Collection Area(um)', ylabel=f'Recon. Convergence Time Constant(s)', title_pad=title_pad)
    set_tick(ax, y_tick_integer=False)  # Normalized
    set_spines(ax)
    set_legend(ax, font_size=17, location="upper right")
    plt.tight_layout()
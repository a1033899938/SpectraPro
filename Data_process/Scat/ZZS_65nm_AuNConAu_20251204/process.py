...
filepath = r"D:\ExpData\NP_scattering_Dispersion\ZZS_AuNC_70nm_20251204\ZZS_AuNC_70nm_20251204.h5"
...
with h5py.File(filepath, "a") as f:
    for iType, process_type in enumerate(process_types):
        ...
        root_folder = f[f"OceanOpticsSpectrometer/20251204/"] # Tile_1/65nmAuNC_4/Image/
        ...
        for iTile, tiled_image_key in enumerate(tiled_image_keys):
            ...
            for iParticle, particle_key in enumerate(particle_keys):
                ...
                maxz = np.array(maxz)
                # 分段1：480-70
                fit_section = [[486.1, 656.3], [656.3, 900]]
                show_section = [[450, 656.3], [656.3, 1000]]
                ...
                "最大强度处的z-int曲线"
                wavs_target = [486.1, 587.6, 656.3, 800, 900] + [wav_max]
                wavs_indice_target = [find_val_idx(wav, wav_target) for wav_target in wavs_target] + [wav0_idx]
                colors = ["#0066CC", "#33AA33", "#FF3333", "#CC0066", "#660033", "orange"]
                ...
                for iWavTarget, (color, wav_target, wav_index_target) in enumerate(
                        zip(colors, wavs_target, wavs_indice_target)):

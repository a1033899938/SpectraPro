    ...
    for iType, process_type in enumerate(process_types):
        ...
        for idx in range(2):  # min or max
            for iSection in range(2):
                ...
                counts, bins, patches = ax.hist(x=maxz_range_stats[iType][:, idx+2*iSection], bins=30, color='blue', edgecolor='black',
                                                density=False)
        ...
        print(f"最佳扫描范围(99.99367 % 几率扫描完整的光谱): {process_type}: {np.min(mins):.2f} - {np.max(maxs):.2f} um, range: {(np.max(maxs)-np.min(mins)):.2f} um, Slope: {(total_maxz_range/total_fit_range):.3f} um/nm")

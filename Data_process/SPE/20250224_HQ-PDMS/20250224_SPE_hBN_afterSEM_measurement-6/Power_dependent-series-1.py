...
def load_data(h5_file, dir_name, save_folder=None):
    ...
    with h5py.File(h5_file, "r") as f:
        ...
        for key in data.keys():
            if ("hBN_afterSEM_5kV_5min_60KX_time_series_P" in key
                    and 'hBN_afterSEM_5kV_5min_60KX_time_series_P400uW_0' not in key
                    and 'hBN_afterSEM_5kV_5min_60KX_time_series_P600uW_0' not in key
                    and 'hBN_afterSEM_5kV_5min_60KX_time_series_P1100uW_0' not in key):
                sp_series_name.append(key)
                if key == 'hBN_afterSEM_5kV_5min_60KX_time_series_P400uW_0':
                    powers.append(300)
                else:
                    powers.append(int(key[len("hBN_afterSEM_5kV_5min_60KX_time_series_P"):-len("uW_0")]))
        # 根据powers重新排序
        ...
        bgd = np.array(data[sp_series_name[0]].attrs['background'])
        bgd_time = data[sp_series_name[0]].attrs['background_int'] / 1000
        bgd = bgd / bgd_time
        wav = np.array(data[sp_series_name[0]].attrs['wavelengths'])
...
if __name__ == '__main__':
    ...
    save_folder = os.path.join(os.path.dirname(h5_file), 'series-1', 'curve_fit')
    ...
    y = np.array(cws)
    ax2.errorbar(powers, cws, yerr=err_cws, markersize=10, markerfacecolor='none', markeredgecolor='blue', markeredgewidth=1.5, fmt=fmts[1], color=colors[1], ecolor=colors[1], capsize=10)

    y = np.array(gammas)
    ax3.errorbar(powers, gammas, yerr=err_gammas, markersize=10, markerfacecolor='none', markeredgecolor='blue', markeredgewidth=1.5, fmt=fmts[2], color=colors[2], ecolor=colors[2], capsize=10)

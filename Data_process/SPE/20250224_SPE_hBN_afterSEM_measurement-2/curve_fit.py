...
if __name__ == '__main__':
    ...
    with h5py.File(datapath1, "r") as f:
        ...
        legend_labels_afterSEM = ['3000uW', '2000uW', '1000uW', '500uW', '200uW', '100uW', '50uW',
                                  '20uW', '10uW']
        excitation_power = np.array([3000, 2000, 1000, 500, 200, 100, 50, 20, 10])

        keys_afterSEM.reverse()
        legend_labels_afterSEM.reverse()
        excitation_power = excitation_power[::-1]
        ...
        for i, key in enumerate(keys_afterSEM):
            ...
            try:
                ...
                mag_fits.append((A1/np.pi)/(gamma1/2))
                wav_fits.append(x1)
                gamma_fits.append(gamma1)
                ...
                y_fit2 = lorentzian(x_for_fit, A2, x2, gamma2)
    ...
    plt.tight_layout()
    # from src.general.save_figure import save_subfig
    # plt.savefig(f'D:\\ExpData\\SPE\\20250224_SPE_hBN_afterSEMprocess\\measurement-2\\curve_fit\\power_dependent_magnitude_wavelength_linewidth.png')
    plt.show()
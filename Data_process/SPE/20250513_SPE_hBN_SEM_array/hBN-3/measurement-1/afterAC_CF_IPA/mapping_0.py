...
with h5py.File(datapath1, "r") as f:
    mapping_name = 'hBN_3_afterAC-CF-IPA_mapping_0'
    ...
    for i in range(mapping_range[0]):
        ...
        for j in range(mapping_range[1]):
            ...
            #               [100000, 600, 200,
            #                100000, 650, 100])
            #     x_for_fit, y_for_fit = choose_range(wav, sp, min_val=510, max_val=660)
            #     popt, pcov = curve_fit(double_gaussian, x_for_fit, y_for_fit, p0=p0, bounds=bounds)
            #
            #     A1, mu1, sigma1, A2, mu2, sigma2 = popt
            #
            #     x = x_for_fit
            #     y = double_gaussian(x, *popt)
            #     ax.plot(x, y, '-b')
            #     mapping_5[i][j] = A2
            #     mean_sigma += sigma1
            #     num_sigma += 1
            ...
            mapping[i][j] = sp[find_val_idx(wav, choose_wav)]
            # plt.show()
# print("mean sigma:", mean_sigma / num_sigma)
the_figure(ax)

np.save(os.path.join(os.path.dirname(datapath1), fr'm1\afterAC-CF-IPA_{choose_wav}.npy'), mapping)

"""保存图片"""
save_fig = 0
mapping =np.load(os.path.join(os.path.dirname(datapath1), fr'm1\afterAC-CF-IPA_{choose_wav}.npy'))
...
# if save_fig == 1:
#     fig.savefig(os.path.join(os.path.dirname(datapath1), f'm1\PL_afterAC-CF-IPA.png'))
#     fig0.savefig(os.path.join(os.path.dirname(datapath1), f'm1\PL_mapping_afterAC-CF-IPA.png'))
plt.show()
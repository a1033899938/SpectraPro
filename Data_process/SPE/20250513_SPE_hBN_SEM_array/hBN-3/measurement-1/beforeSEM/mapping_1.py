...
with h5py.File(datapath1, "r") as f:
    mapping_name = 'hBN_3_beforeSEM_mapping_1'
    ...
    for i in range(mapping_range[0]):
        ...
        for j in range(mapping_range[1]):
            ...
            sp = sp / sp_time
            sp = sp - bgd
            # sp = replace_outliers(sp, wav, (610, 620), threshold=5)
...
np.save(os.path.join(os.path.dirname(datapath1), fr'm1\before_SEM_1.npy'), mapping)

"""保存图片"""
save_fig = 0
mapping =np.load(os.path.join(os.path.dirname(datapath1), fr'm1\before_SEM_1.npy'))
...
# if save_fig == 1:
#     fig.savefig(os.path.join(os.path.dirname(datapath1), f'm1\PL_before_SEM_1.png'))
#     fig0.savefig(os.path.join(os.path.dirname(datapath1), f'm1\PL_mapping_before_SEM_1.png'))
plt.show()
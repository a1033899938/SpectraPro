...
with h5py.File(datapath1, "r") as f:
    mapping_name = 'hBN_1-1_afterSEM_mapping_0'
    ...
    for i in range(mapping_range[0]):
        ...
        for j in range(mapping_range[1]):
            ...
            # mapping_5[i][j] = sp[min_index_for_fit]
the_figure(ax)

np.save(os.path.join(os.path.dirname(datapath1), fr'm1\after_SEM.npy'), mapping)

"""保存图片"""
save_fig = 1
mapping =np.load(os.path.join(os.path.dirname(datapath1), fr'm1\after_SEM.npy'))

...
datapath1 = r"D:\ExpData\SPE\20250513_SPE_hBN_SEM_array\hBN-1-1\measurement-1.h5"

def the_figure(ax):
    """拟合曲线"""
    set_figure.set_label_and_title(ax, title=f'', ylabel='Intensity(counts)')
    set_figure.set_spines(ax)
    set_figure.set_tick(ax, ticks_xlabel=np.arange(450, 651, 50))  # Normalized
    plt.tight_layout()


def the_figure1(ax, cbar):
    """拟合曲线"""
    set_figure.set_label_and_title(ax, title=f'', xlabel='X', ylabel='Y', colorbar=cbar, colorbar_label='Intensity(counts)')
    set_figure.set_spines(ax)
    set_figure.set_tick(ax, colorbar=cbar)  # Normalized
    plt.tight_layout()

# hBN_1-1_beforeSEM_mapping_0
# hBN_1-2_beforeSEM_mapping_0
# hBN_1-1_afterSEM_mapping_0
# hBN_1-2_afterSEM_mapping_0
# hBN_1-1_afterAC-CF-IPA_mapping_0
# hBN_1-2_afterAC-CF-IPA_mapping_0
"""fig0"""
with h5py.File(datapath1, "r") as f:
    mapping_name = 'hBN_1-1_beforeSEM_mapping_0'
    ...
    for i in range(mapping_range[0]):
        ...
        for j in range(mapping_range[1]):
            ...
            sp = sp - bgd
            sp = sp / sp_time
            x, y = choose_range(wav, sp, min_val=460, max_val=660)
            y = remove_spikes_with_local_median(x, y, filter_size=5)
            ax.plot(x, y)


            mapping[i][j] = np.mean(sp[find_val_idx(wav, 505):find_val_idx(wav, 600)])
            # mapping[i][j] = sp[min_index_for_fit]
the_figure(ax)

np.save(os.path.join(os.path.dirname(datapath1), fr'm1\before_SEM.npy'), mapping)

"""保存图片"""
save_fig = 1
mapping =np.load(os.path.join(os.path.dirname(datapath1), fr'm1\before_SEM.npy'))
...
if save_fig == 1:
    fig.savefig(os.path.join(os.path.dirname(datapath1), f'm1\PL_beforeSEM.png'))
    fig0.savefig(os.path.join(os.path.dirname(datapath1), f'm1\PL_mapping_beforeSEM.png'))
plt.show()
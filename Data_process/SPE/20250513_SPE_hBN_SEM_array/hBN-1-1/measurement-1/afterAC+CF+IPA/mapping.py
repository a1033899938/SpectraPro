...
with h5py.File(datapath1, "r") as f:
    mapping_name = 'hBN_1-1_afterAC-CF-IPA_mapping_0'
...
np.save(os.path.join(os.path.dirname(datapath1), fr'm1\after_AC_CF_IPA_{choose_wav}.npy'), mapping)

"""保存图片"""
save_fig = 1
mapping =np.load(os.path.join(os.path.dirname(datapath1), fr'm1\after_AC_CF_IPA_{choose_wav}.npy'))
...
if save_fig == 1:
    fig.savefig(os.path.join(os.path.dirname(datapath1), f'm1\PL_after_AC_CF_IPA.png'))
    fig0.savefig(os.path.join(os.path.dirname(datapath1), f'm1\PL_mapping_after_AC_CF_IPA.png'))
plt.show()
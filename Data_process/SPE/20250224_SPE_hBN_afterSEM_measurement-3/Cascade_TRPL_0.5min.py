...
if __name__ == '__main__':
    ...
    with h5py.File(datapath1, "r") as f:
        ...
        keys_30s_time_resolution = []
        keys_30s_time_resolution.append('0.5min_10uW_time_series_0')
        keys_30s_time_resolution.append('0.5min_20uW_time_series_0')
        keys_30s_time_resolution.append('0.5min_50uW_time_series_0')
        keys_30s_time_resolution.append('0.5min_100uW_time_series_0')
        keys_30s_time_resolution.append('0.5min_200uW_time_series_0')
        keys_30s_time_resolution.append('0.5min_500uW_time_series_0')
        keys_30s_time_resolution.append('0.5min_1000uW_time_series_0')
        keys_30s_time_resolution.append('0.5min_2000uW_time_series_0')
        keys_30s_time_resolution.append('0.5min_2_3000uW_time_series_0')
        keys_30s_time_resolution.append('0.5min_2_2000uW_back_time_series_0')
        keys_30s_time_resolution.append('0.5min_2_1000uW_back_time_series_0')
        keys_30s_time_resolution.append('0.5min_2_500uW_back_time_series_0')
        keys_30s_time_resolution.append('0.5min_2_200uW_back_time_series_0')
        keys_30s_time_resolution.append('0.5min_2_100uW_back_time_series_0')
        keys_30s_time_resolution.append('0.5min_2_50uW_back_time_series_0')
        keys_30s_time_resolution.append('0.5min_2_20uW_back_time_series_0')
        keys_30s_time_resolution.append('0.5min_2_10uW_back_time_series_0')

        titles_30s_time_resolution = ['10uW', '20uW', '50uW', '100uW', '200uW', '500uW', '1000uW', '2000uW', '3000uW',
                                  '2000uW_back', '1000uW_back', '500uW_back', '200uW_back', '100uW_back', '50uW_back',
                                  '20uW_back', '10uW_back']
        # plt.close('all')
        for idx, key in enumerate(keys_30s_time_resolution):
            ...
            # if key == '1min_10uW_back_time_series_0':
            #     sps[sps > 12] = 0

            x = wav
            y = np.arange(len(keys_30s_time_resolution))
            Z = np.array(sps)
            ...
            """手动去噪"""
            val = 0
            if key == '0.5min_10uW_time_series_0':
                for row in range(Z.shape[0]):
                    Z[row, :] = replace_outliers(Z[row, :], (700, 800), 5)
                    Z[row, :] = replace_outliers(Z[row, :], (1000, 1100), 5)

            elif key == '0.5min_20uW_time_series_0':
                for row in range(Z.shape[0]):
                    Z[row, :] = replace_outliers(Z[row, :], (400, 500), 5)
                    Z[row, :] = replace_outliers(Z[row, :], (1000, 1100), 5)
            elif key == '0.5min_50uW_time_series_0':
                for row in range(Z.shape[0]):
                    Z[row, :] = replace_outliers(Z[row, :], (900, 1000), 5)
                Z[19, :] = replace_outliers(Z[row, :], (680, 690), 25)
            elif key == '0.5min_100uW_time_series_0':
                for row in range(Z.shape[0]):
                    Z[row, :] = replace_outliers(Z[row, :], (1000, 1100), 5)
            elif key == '0.5min_200uW_time_series_0':
                for row in range(Z.shape[0]):
                    Z[row, :] = replace_outliers(Z[row, :], (750, 800), 20)
                    Z[row, :] = replace_outliers(Z[row, :], (900, 1000), 5)
            elif key == '0.5min_1000uW_time_series_0':
                for row in range(Z.shape[0]):
                    Z[row, :] = replace_outliers(Z[row, :], (900, 1000), 5)
            elif key == '0.5min_2_1000uW_back_time_series_0':
                for row in range(Z.shape[0]):
                    Z[row, :] = replace_outliers(Z[row, :], (399, 440), 5)
                    Z[row, :] = replace_outliers(Z[row, :], (800, 900), 5)
                    Z[row, :] = replace_outliers(Z[row, :], (1000, 1100), 5)
            elif key == '0.5min_2_500uW_back_time_series_0':
                for row in range(Z.shape[0]):
                    Z[row, :] = replace_outliers(Z[row, :], (700, 800), 5)
                    Z[row, :] = replace_outliers(Z[row, :], (900, 1000), 5)
            elif key == '0.5min_2_200uW_back_time_series_0':
                for row in range(Z.shape[0]):
                    Z[row, :] = replace_outliers(Z[row, :], (700, 800), 5)
                    Z[row, :] = replace_outliers(Z[row, :], (580, 600), 18)
            elif key == '0.5min_2_100uW_back_time_series_0':
                for row in range(Z.shape[0]):
                    Z[row, :] = replace_outliers(Z[row, :], (700, 800), 5)
                    Z[row, :] = replace_outliers(Z[row, :], (1000,1100), 5)
            elif key == '0.5min_2_50uW_back_time_series_0':
                for row in range(Z.shape[0]):
                    Z[row, :] = replace_outliers(Z[row, :], (1000, 1100), 5)
                    Z[row, :] = replace_outliers(Z[row, :], (480, 520), 5)
                    Z[row, :] = replace_outliers(Z[row, :], (550, 600), 14)
            elif key == '0.5min_2_10uW_back_time_series_0':
                for row in range(Z.shape[0]):
                    Z[row, :] = replace_outliers(Z[row, :], (400, 500), 3)
                    Z[row, :] = replace_outliers(Z[row, :], (700, 800), 5)
                    Z[row, :] = replace_outliers(Z[row, :], (1000, 1100), 5)
            ...
            title = f'hBN-after-SEM-process_0.5min Time-Resolved PL\n{titles_30s_time_resolution[idx]}'
            ...
            # plt.savefig(f'D:\\ExpData\\SPE\\20250224_SPE_hBN_afterSEMprocess\\measurement-3\\0.5min_time_resolved_PL\\{titles_30s_time_resolution[idx]}.png')

...
ax.scatter(rots, ints, linewidth=3, color='#1f77b4')
...
ax.plot(rots, y_fit, linewidth=2, color='#1f77b4')
ax.plot(rots, y_fit, linewidth=2, color='#1f77b4')
...
folder_path = r"D:\ExpData\SPE\20250224_SPE_hBN_afterSEMprocess\ARS\20250318\SPEPolarization\m1"
extensions = ['.spe']
...
for file_path, file_name in zip(file_paths, file_names):
    ...
    min_differences = np.abs(x - 505)
    min_index = np.argmin(min_differences)
    max_differences = np.abs(x - 625)
    max_index = np.argmin(max_differences)
...
# fig = plt.figure(figsize=(12, 9))
# ax = fig.add_subplot(111, polar=True)
ax.scatter(rots, ints, linewidth=3, color='#d62728')

set_figure.set_label_and_title(ax, title = 'Excitation & Emission-Polarization', xlabel='', ylabel='', label_pad=25)
# set_figure.set_spines(ax0)
# set_figure.set_tick(ax0, xbins=16, ybins=10, fontsize=10, fontweight='bold',
#                     linewidth=3, tick_pad=5, direction='in',
#                     ticks_xlabel=np.arange(300, 1101, 50))  # Normalized
...
if save_fig == 1:
    from src.general.save_figure import save_subfig
    plt.savefig(fr"D:\ExpData\SPE\20250224_SPE_hBN_afterSEMprocess\ARS\20250318\Polarization.png")

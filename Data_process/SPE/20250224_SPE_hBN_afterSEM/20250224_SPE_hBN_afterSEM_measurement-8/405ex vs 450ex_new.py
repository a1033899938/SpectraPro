...
def load_data(h5_file, dir_name, sp_names, les):
    ...
    # 创建画布
    sps = np.array(sps)
    fig, axes = cascade_2d(x=x, ys=sps, les=les, figsize=(8, 6))

    for i, ax in enumerate(axes):
        set_tick(ax, linewidth=1, ybins=4, fontsize=10, show_ylabel_every_ticks=2)
        if i == 0:
            set_label_and_title(axes[i], title='', xlabel='', ylabel='Intensity(cts)', ylabel_ha='center',
                                ylabel_va='center', y_label_position=(-0.15, 0.9))

    title = f''
    set_label_and_title(axes[0], title=title, xlabel='', ylabel='', title_pad=30)
    set_label_and_title(axes[-1], title='', ylabel='', x_label_pad=20)
    set_tick(axes[-1], ticks_xlabel=np.arange(400, 901, 100), linewidth=1, ybins=4, fontsize=10,
             show_xlabel_every_ticks=2, show_ylabel_every_ticks=2)
...
def the_figure(ax, le):
    """拟合曲线"""
    set_label_and_title(ax, title=f'405 / 450 nm Laser Excitation', ylabel='Intensity(cts)')
    set_spines(ax)
    set_tick(ax, ticks_xlabel=np.arange(400, 901, 100))  # Normalized
    set_legend(ax, legend_labels=le)

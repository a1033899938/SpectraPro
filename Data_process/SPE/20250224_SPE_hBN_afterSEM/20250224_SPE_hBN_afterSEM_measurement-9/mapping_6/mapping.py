...
if __name__ == '__main__':
    ...
    # 创建文件夹
    save_folder = os.path.join(os.path.dirname(h5_file), f'{group_name}')
    if not os.path.exists(save_folder):
        os.makedirs(save_folder, exist_ok=True)

    # 保存mapping数据
    npz_file = os.path.join(save_folder, f'PL_mapping.npy')
    ...
    # 保存图片到npz的文件夹下
    fig_mapping.savefig(os.path.join(save_folder, f'mapping_2.png'))
    fig_graph.savefig(os.path.join(save_folder, f'graph.png'))
    plt.show()
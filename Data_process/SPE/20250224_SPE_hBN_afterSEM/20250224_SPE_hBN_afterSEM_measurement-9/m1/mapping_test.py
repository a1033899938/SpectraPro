...
datapath1 = r"D:\ExpData\SPE\20250224_SPE_hBN_afterSEMprocess\measurement-9\20250224_SPE_hBN_afterSEM_measurement-9.h5"

"""fig0"""
with h5py.File(datapath1, "r") as f:
    mapping_name = 'hBN_afterSEM_mapping_3'
    ...
    peak1_index = np.argmin(np.abs(wav - 538))
    ...
    for i in range(mapping_range[0]):
        ...
        for j in range(mapping_range[1]):
            ...
            # mapping[i][j] = sp[peak1_index] if sp[peak1_index] <5000 else 0
            mapping[i][j] = sp[peak1_index]

np.save(os.path.join(os.path.dirname(datapath1), fr'm4\mapping_538.npy'), mapping)

save_fig = 0
mapping =np.load(os.path.join(os.path.dirname(datapath1), fr'm4\mapping_538.npy'))

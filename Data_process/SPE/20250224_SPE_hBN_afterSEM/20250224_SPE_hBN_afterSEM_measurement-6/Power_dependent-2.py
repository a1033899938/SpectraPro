...
def the_figure(ax, key):
    ...
    set_figure.set_tick(ax, ticks_xlabel=np.arange(400, 701, 50))
...
def the_figure1(ax1):
    ...
    set_figure.set_tick(ax1, ybins=0, ticks_xlabel=np.arange(0, 3001, 500))
...
#         if ('hBN_afterSEM_5kV_5min_60KX_time_series' in key
#                 and key != 'hBN_afterSEM_5kV_5min_60KX_time_series_P600uW_0'
#                 and key != 'hBN_afterSEM_5kV_5min_60KX_time_series_P1100uW_0'
#                 and 'hBN_afterSEM_5kV_5min_60KX_time_series_m' not in key):
#             sp = data[key]
...
"""保存PL线形拟合数据"""
# save_lines_txt(powers, ints, errors, save_full_path=os.path.join(os.path.dirname(datapath1), 'powers_ints-2.txt'))
...
plt.tight_layout()
if save_fig == 1:
    # plt.savefig(os.path.join(os.path.dirname(datapath1), 'curve_fit-2', f'power_dependent_intensity_of_peak-1_log.png'))

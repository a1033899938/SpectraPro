...
def load_data(h5_file, dir_name, sp_names, le=None, save_folder=None):
    with h5py.File(h5_file, "r") as f:
        ...
        for i, key in enumerate(sp_names):
            ...
            sp_max = np.max(sp)
            sp = sp/sp_max  # com
            x, y = choose_range(wav, sp, x1=400, x2=700)
            ax.plot(x, y, label='Exp. data')
            # 曲线拟合
            x_for_fit, y_for_fit = choose_range(wav, sp, x1=510, x2=700)
            # fitting_paras = triple_peaks_fitting(x_for_fit, y_for_fit, ax=ax)
            fitting_paras = quatra_peaks_fitting_voigt(x_for_fit, y_for_fit, ax=ax, maxfev=10000)  # com
            # # 储存拟合参数
            # ints.append(fitting_paras["peak1"]["magnitude"])
            # cws.append(fitting_paras["peak1"]["center"])
            # gammas.append(fitting_paras["peak1"]["fwhm"])

            # ints.append(fitting_paras["peak4"]["magnitude"]*sp_max)  # com
            # cws.append(fitting_paras["peak4"]["center"])  # com
            # gammas.append(fitting_paras["peak4"]["fwhm"])  # com

            # the_figure(ax, title=f"{le[i]}uW
...
def the_figure(ax, title):
    """拟合曲线"""
    set_figure.set_label_and_title(ax, title=title, ylabel='Normalized Intensity(cts)')
    set_figure.set_spines(ax)
    set_figure.set_tick(ax, ticks_xlabel=np.arange(400, 701, 50), ticks_ylabel=np.arange(0, 1.1, 0.2), show_ylabel_every_ticks=2)  # Normalized
    set_legend(ax, font_size=13)
    plt.tight_layout()

...
def load_data(h5_file, dir_name, save_folder=None):
    ...
    with h5py.File(h5_file, "r") as f:
        ...
        for key in data.keys():
            if ("hBN_afterSEM_5kV_5min_60KX_time_series_m5_P" in key
                    and 'hBN_afterSEM_5kV_5min_60KX_time_series_m5_P1000uW_1' not in key
                    and 'hBN_afterSEM_5kV_5min_60KX_time_series_m5_P2000uW_1' not in key
                    and 'hBN_afterSEM_5kV_5min_60KX_time_series_m5_P1000uW_2' not in key
                    and 'hBN_afterSEM_5kV_5min_60KX_time_series_m4_P5000uW_2' not in key
                    and 'hBN_afterSEM_5kV_5min_60KX_time_series_m5_P2000uW_2' not in key
                    and 'hBN_afterSEM_5kV_5min_60KX_time_series_m5_P3000uW_1' not in key
            and 'hBN_afterSEM_5kV_5min_60KX_time_series_m5_P5000uW_1' not in key
            and 'hBN_afterSEM_5kV_5min_60KX_time_series_m5_P5000uW_2' not in key
            and 'hBN_afterSEM_5kV_5min_60KX_time_series_m5_P7000uW_1' not in key
            and 'hBN_afterSEM_5kV_5min_60KX_time_series_m5_P7000uW_2' not in key
            and 'hBN_afterSEM_5kV_5min_60KX_time_series_m5_P7000uW_3' not in key
            and 'hBN_afterSEM_5kV_5min_60KX_time_series_m5_P7000uW_4' not in key
            and 'hBN_afterSEM_5kV_5min_60KX_time_series_m5_P5000uW_3' not in key):
                sp_series_name.append(key)
                powers.append(int(key[len("hBN_afterSEM_5kV_5min_60KX_time_series_m5_P"):-len("uW_0")]))
        # 根据powers重新排序
        ...
        for i, key in enumerate(sp_series_name):
            ...
            for row in range(sp_2d.shape[0]):
                ...
                # 储存拟合参数
                ints_row.append(fitting_paras[0])
                cws_row.append(fitting_paras[1])
                gammas_row.append(fitting_paras[2])
            ...
            ints.append(np.mean(ints_row[1:-1]))  # 去掉最小值和最大值再平均
            err_ints_row = np.max(ints_row[1:-1]) - np.min(ints_row[1:-1])  # 去掉最小值和最大值再取range
            err_ints.append(err_ints_row)

            # 中心波长与err
            cws_row = sorted(cws_row)
            cws.append(np.mean(cws_row[1:-1]))
            err_cws_row = np.max(cws_row[1:-1]) - np.min(cws_row[1:-1])
            err_cws.append(err_cws_row)

            # 线宽与err
            gammas_row = sorted(gammas_row)
            gammas.append(np.mean(gammas_row[1:-1]))
            err_gammas_row = np.max(gammas_row[1:-1]) - np.min(gammas_row[1:-1])
...
def the_figure1(ax1):
    ...
    set_figure.set_tick(ax1, ticks_xlabel=np.arange(0, 7001, 1000))  # Normalized
...
def the_figure2(ax2):
    ...
    set_figure.set_tick(ax2, ticks_xlabel=np.arange(0, 7001, 1000),
...
def the_figure3(ax3):
    ...
    set_figure.set_tick(ax3, ticks_xlabel=np.arange(0, 7001, 1000), ticks_ylabel=np.arange(0, 11, 1))
    plt.tight_layout()

if __name__ == '__main__':
    """路径"""
    h5_file = r"D:\ExpData\SPE\20250224_SPE_hBN_afterSEMprocess\measurement-6\measurement-6.h5"
    save_folder = os.path.join(os.path.dirname(h5_file), 'series-5', 'curve_fit')

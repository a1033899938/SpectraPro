...
if __name__ == '__main__':
    ...
    with h5py.File(datapath1, "r") as f:
        ...
        for index, key in enumerate(keys_afterSEM):
            if key != 'hBN_afterSEM_5kV_5min_expose10s_50uW__0':
                continue
            sp = data[key]
    ...
    set_figure.set_label_and_title(ax0, title=title, xlabel='Process time(min)', ylabel='Normalized Intensity(a.u.)',
    ...
    with h5py.File(datapath1, "r") as f:
        ...
        power_withoutSEM = [10, 20, 50, 100, 200, 500, 1000, 2000, 3000]

        ax1 = fig0.add_subplot(122, projection='3d')
        sps = []
        for index, key in enumerate(keys_withoutSEM):
            print(key)
            sp = data[key]
            bgd = np.array(sp.attrs['background'])
            wav = np.array(sp.attrs['wavelengths'])
    ...
    set_figure.set_label_and_title(ax1, title=title, xlabel='Process time(min)', ylabel='Normalized Intensity(a.u.)',
                                   label_fontsize=25, title_fontsize=25,
                                   label_font_family='Times New Roman', title_font_family='Times New Roman',
                                   label_fontweight='bold', title_fontweight='bold',
                                   label_pad=15, title_pad=15, mode='3d')
    ...
    ax1.set_zlabel(zlabel='Normalized Intensity(a.u.)', rotation=90, labelpad=15)
    ...
    # plt.tight_layout()
    plt.savefig(r'D:\ExpData\SPE\20250224_SPE_hBN_afterSEMprocess\measurement-2\\cascade&2d\\20250224_SPE_hBN_afterSEM_measurement-2_normalized_by_power.png')
    plt.show()
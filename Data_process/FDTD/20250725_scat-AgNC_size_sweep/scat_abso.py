...
filepath1 = r"D:\FDTDSimFile\NP_scattering\size_of_AgNC-lambda_scat_abso\lambda_scats_absos.txt"
data1 = read_lines_txt(filepath1, separator=',', skip_lines=1, row_lines_names=0)
filepath2 = r"D:\FDTDSimFile\NP_scattering\size_of_AgNC-lambda_scat_abso\lens.txt"
...
wc = MyColor.get_color_series("professional")
...
for i, (key, value) in enumerate(data1.items()):
    ...
    if 'scat' in key:
        scat = np.array(value)
        ax1.plot(wav, scat, label=le, color=wc[num-1])
        if scats is None:
            scats = scat
        else:
        ...
        abso = -np.array(value)
        ax2.plot(wav, abso, label=le, color=wc[num-1])
        if absos is None:
            absos = abso
        else:

...
if __name__ == '__main__':
    ...
    with h5py.File(datapath1, "r") as f:
        ...
        for key in data.keys():
            ...
            sp = np.array(sp) - bgd
            if '80nm' in key:
                ax1.plot(wav, sp/np.max(sp), color='gray')
                legend_labels.append(key + ': DF')

                popt, pcov = curve_fit(double_gaussian, wav, sp / np.max(sp), p0=[1, 600, 50, 1, 750, 50])
                ...
                legend_labels.append(key + ': PL')

                popt, pcov = curve_fit(gaussian, wav, sp / np.max(sp), p0=[1, 750, 50])
    ...
    title = 'Dark Field Scattering / WSe2 ML PL'
    ...
    set_figure.set_tick(ax1, xbins=6, ybins=10, fontsize=15, fontweight='bold',
                        ...
                        ticks_ylabel=np.linspace(0, 1, 6))  # Normalized
    ...
    #
    # title = 'WSe2/Aufilm'
    # set_figure.set_label_and_title(ax2, title=title, ylabel='Intensity(a.u.)',
    #                                label_fontsize=25, title_fontsize=25,
    #                                label_font_family='Times New Roman', title_font_family='Times New Roman',
    #                                label_fontweight='bold', title_fontweight='bold',
    #                                label_pad=8, title_pad=15)
    ...
    #                     ticks_xlabel=np.linspace(400, 1100, 8),
    #                     ticks_ylabel=np.linspace(0, 1, 6))  # Normalized
    # set_figure.set_legend(ax2, legend_labels=legend_labels, font_size=5, location='upper right')

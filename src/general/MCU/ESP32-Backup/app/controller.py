...
class DeviceController():
    ...
    def print_at_PC(self, data):
        if not data.strip():
            return len(data)
        # 标准化系统日志格式（SESSION=0）
        log_line = f"LOG: {data.strip()}\r\n"
        self.uart.write(log_line.encode('utf-8'))
        return len(data)
    
    def _initialize_components(self):
        """初始化所有组件"""
        # 初始化LED
        self.led = LEDController(self.cfg._led_pin)
        
        # 初始化UART通信
        self.comm = UARTCommunication(self.cfg)
        print(self.comm.error_info)
        ...
        try:
            while self.running:
                ...
                if self.comm and self.comm.initialized:  # 
                    ...
                    print_level(2, 'uart', "等待UART初始化x成...")
                    time.sleep(1)
                    continue
        ...
        try:
            ...
            if len(parts) == 3 and parts[0] == "INQUIRE" and parts[1] == "SESSION":
                ...
                if session_id in self.executed_sessions:
                    ...
                    self.comm.send_data(response_msg)

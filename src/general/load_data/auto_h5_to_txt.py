            ...
            if isinstance(f[key], h5py._hl.group.Group):
                ...
                try:
                    ...
                    if "integration_time" in data.attrs:  # 视为光谱文件
                        if "number of spectra" in data.attrs:  # 视为时间序列光谱
                            sp_num = data.attrs.get('number of spectra')

                            sp = np.array(data)
                            sp_time = np.array(data.attrs.get('integration_time', [])) / 1000
                            sp = sp / sp_time
                            print(np.shape(sp[0]))
                            ...
                            sps = [sp[i] for i in range(sp_num)]
                            sps_names = [f"sp-{i}" for i in range(sp_num)]

                            datas = [*sps, bgd, ref, wav]
                            lines_names = [*sps_names, "bgd", "ref", "wav"]
                            ...
                            datas, lines_names = zip(*filtered) if filtered else ([], [])
                            save_path = os.path.join(folder_now, f"{key}.txt")
                            save_lines_txt(*datas, save_path=save_path, separator=' ', lines_names=lines_names,
                                           confirm_all_overwrite=False)
